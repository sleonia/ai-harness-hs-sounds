...
- `./install.sh`: build the plugin and install it as a local marketplace under `~/.claude/ai-harness-hs-sounds/`. After running, register it inside Claude Code with `/plugin marketplace add ~/.claude/ai-harness-hs-sounds` then `/plugin install hearthstone-sounds@ai-harness-hs-sounds`.
...
local marketplace at `~/.claude/ai-harness-hs-sounds/`:

```
~/.claude/ai-harness-hs-sounds/
├── .claude-plugin/
...
The user then runs `/plugin marketplace add ~/.claude/ai-harness-hs-sounds` and
`/plugin install hearthstone-sounds@ai-harness-hs-sounds` once inside Claude Code.

### Plugin Architecture (Built Distribution)

- **Location**: `dist/claude/` directory (after build); `~/.claude/ai-harness-hs-sounds/hearthstone-sounds/` (after install)
