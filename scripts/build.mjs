 ...
 *
 * Args:
 *   agent - Optional agent name to build (default: 'claude')
 */
async function build(agent = 'claude') {
  ...
  for (const { path: p, type, name } of requiredPaths) {
    const exists = type === 'dir' ? fs.existsSync(p) && fs.statSync(p).isDirectory() : fs.existsSync(p);
    if (!exists) {
  ...
  try {
    ...
    // 4. Copy soundpack/ → dist/claude/soundpack/ (recursive)
  ...
  for (const file of files) {
    ...
    if (file.isDirectory()) {
      ...
      if (content !== originalContent) {
        ...
        console.log(`  Updated imports in: ${path.relative(baseDir, fullPath)}`);
      }
    }
  }
}
